...
def _extract_tickers_from_query(query: str) -> List[str]:
    ...
    tickers += re.findall(r"\b\d{6}\b", query)
...
def _normalize_kr_tickers(tickers: List[str]) -> List[str]:
    ...
    #  - 아니면 원본 유지
    # ----------------------------------------------------------------------------
    tickers = [f"{ticker}.KS" if re.fullmatch(r"\d{6}", ticker) else ticker for ticker in tickers]
    return tickers
...
def before_model_callback(
    ...
    **kwargs,
    ...
    try:
        ...
        if last.role != "user": # 마지막 대화 주체가 사용자인가?
            raise ValueError("사용자 메시지가 아닙니다.")
        query = last.parts[0].text # 사용자 메시지 텍스트 추출
        payload = _handle(query) # 페이로드 생성
        body_md = render_day1(query, payload)
        saved = save_markdown(query=query, route="day1", markdown=body_md) # 파일 저장
        md = render_enveloped(kind="day1", query=query, payload=payload, saved_path=saved) # 마크다운 렌더
        return LlmResponse(content=types.Content(parts=[types.Part(text=md)], role="model")) # 응답
    except Exception as e:
        return LlmResponse(content=types.Content(parts=[types.Part(text=f"Day1 에러: {e}")]))
